...
void ofApp::update() {
	...
	for (int x = -300; x < 0; x += 300) {
		...
		for (int y = -450; y <= 450; y += 300) {
			...
			i++;
			for (auto radius = 60; radius <= 140; radius += 10) {

				glm::highp_mat4 rotation;
				auto noise_value = ofNoise(y, ofGetFrameNum() * 0.006 - radius * 0.0005);
				...
				this->setRingToMesh(this->face, this->frame, glm::vec3(i % 2 ? x : -x, y, 0), radius, 5, 5, rotation);
			}
		}
	}
}
